...
> Web Serial API は `localhost` または HTTPS で動作します。USBセンサーを使う場合も、この起動方法がおすすめです。
...
アプリを開き、「距離センサー」→「RealSense」を選択して `ws://localhost:8765` に接続する。

macOS / Linux の仮想環境有効化:
...
このプロジェクトの現状スタックは、用途に対してかなり相性が良いです。

- **HTML / CSS / JavaScriptのみ**: 展示・実験・センサー連携アプリとして配布しやすく、ビルド不要で壊れにくい
- **Web Audio API**: 心音のタイミング制御に向いている
- **Web Serial API**: USBマイコン連携をブラウザだけで完結できる
- **WebSocket API**: WiFi距離センサーやRealSenseサーバーとのリアルタイム連携に向いている
- **Canvas API**: ECG波形の軽量描画に十分
- **Python + pyrealsense2**: RealSense連携だけをローカル補助サーバーに切り出せていて責務が明確

今の規模ならReact/Vite等へ移行するメリットはまだ小さめです。UI部品がさらに増える、状態管理が複雑になる、複数画面化する、という段階になったらViteへの移行を検討するとよいです。
